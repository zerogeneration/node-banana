...
`@zerogeneration/providers/node-banana` moves **here**, into the fork, as an
isolated module.
...
(`@zerogeneration/engine-client`).

## Layout

| File | Role |
|---|---|
| `contract.ts` | The neutral engine contract: node-banana's own port-shaped *request* types + node-banana view types declared here; the engine *wire* types (`EngineJob`/`EngineAsset`/the `Engine*Body` bodies) are **aliases over `@zerogeneration/engine-client`** (plan §4.3 — no vendored copy to drift). |
...
`@zerogeneration/providers/node-banana` library-embed. Those three providers run
...
are now **closed** by the `@zerogeneration/engine-client` contract:
...
- ✅ **Vendored `contract.ts` retired** — the engine wire/body types are now
  aliases over `@zerogeneration/engine-client`; the private
  `@zerogeneration/providers` dependency is dropped.
...
- ⏳ **Playground retirement** (other repo) — delete the leftover doc references;
  the `@zerogeneration/providers/node-banana` subpath is already gone.
